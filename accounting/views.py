...
from .models import Transaction, Gst, Ledger, Category
...
def add_transaction(request):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            balance=balance+transaction.amount if transaction.type=="Income" else balance-transaction.amount
            Ledger.objects.update_or_create(
                transaction=transaction,
                defaults={"date":transaction.date,"current_balance":balance}
            )
            return redirect("add_transaction")
    else:
        form=TransactionForm()
    return render(request,"accounting/add_transaction.html",{"form":form,"gst_amount":gst_amount})
...
def categorize_transactions(request):
    ...
    return render(request,"accounting/categorize_transactions.html",{"transactions":transactions,"categories":categories})
...
def generate_gst_bill(request,transaction_id):
    ...
    buf=BytesIO();doc=SimpleDocTemplate(buf);styles=getSampleStyleSheet()
    ...
    doc.build([Paragraph("GST Bill",styles["Title"]),tbl]);buf.seek(0)
    return FileResponse(buf,as_attachment=True,filename=f"GST_Bill_{t.id}.pdf")


def generate_total_gst_bill(request):
    tx=Transaction.objects.all()
    taxable=Decimal("0");gst=Decimal("0")
    for t in tx:
    ...
    total=taxable+gst
    buf=BytesIO();doc=SimpleDocTemplate(buf);styles=getSampleStyleSheet()
    tbl=Table([["Taxable",f"₹{taxable:.2f}"],["GST",f"₹{gst:.2f}"],["Grand Total",f"₹{total:.2f}"]])
    tbl.setStyle(TableStyle([("GRID",(0,0),(-1,-1),1,colors.black)]))
    doc.build([Paragraph("Total GST Bill",styles["Title"]),tbl]);buf.seek(0)
    return FileResponse(buf,as_attachment=True,filename="Total_GST_Bill.pdf")
...
def generate_gst_report(request):
    ...
    doc.build([
        Paragraph("GST Report", styles["Title"]),
        table
    ])
